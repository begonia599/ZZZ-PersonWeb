...
def increment_visitor_count():
    ...
    try:
        ...
        if not metrics_row:
            # 如果不存在，初始化它并设置计数为 1
            new_metrics = WebsiteMetrics(id=1, visitor_count=1, startup_time=get_beijing_now())
            db.session.add(new_metrics)
            db.session.commit()
            ...
            metrics_row.last_updated = get_beijing_now() # 更新最后更新时间
            db.session.commit()
...
def get_website_uptime():
    ...
    try:
        ...
        if metrics_row and metrics_row.startup_time:
            startup_time_beijing = metrics_row.startup_time
            # 如果数据库中的时间是 naive datetime，需要添加时区信息
            if startup_time_beijing.tzinfo is None:
                startup_time_beijing = startup_time_beijing.replace(tzinfo=BEIJING_TZ)
            # 计算运行时间（秒）
            uptime_seconds = (get_beijing_now() - startup_time_beijing).total_seconds()
            return jsonify({"startupTime": startup_time_beijing.isoformat(), "uptimeSeconds": uptime_seconds}), 200
        else:
